...
- Tries `require()` on the `.node` file — if it throws, logs `require_failed` (chicken-and-egg: can't read ABI without loading, can't load due to ABI mismatch). If it succeeds but `.versions.modules` is undefined, that's normal — **better-sqlite3 does NOT expose `.versions.modules`**, so rebuild decision is based on `require()` success/failure, not version string comparison.
...
- **Attempt 1 — `prebuild-install` directly** (preferred): runs `node prebuild-install/bin.js` in the copied `better-sqlite3/` directory. `prebuild-install` downloads the correct prebuilt binary matching the user's ABI — no compilation needed, works across all Node.js versions. The `prebuild-install` package must be in `EXTERNAL` list in `build-server.js` so all transitive deps (`rc`, `detect-libc`, etc.) are bundled.
- **Attempt 2 — `npm rebuild better-sqlite3`** (fallback): if `prebuild-install` fails (no matching prebuilt binary, network unavailable), compiles from C++ source via `node-gyp`. Requires `package.json` in CWD (modern npm ≥ 10 refuses without it). **Known failure on Node 25+**: V8 headers use C++20 designated initializers (`{internal::kFirstEmbedderDataTag,`) that Apple clang rejects with `-std=c++17`. If both attempts fail, the server logs `FAILED: Both prebuild-install and npm rebuild failed.` and the frontend shows an error card.
