  ...
  return (
    <div className="relative" ref={menuRef}>
      <Button
        variant="outline"
        size="sm"
        className={`h-9 px-3 font-bold text-muted-foreground border-border/50 hover:bg-muted/50 cursor-pointer relative ${iconOnly ? 'px-2.5' : ''}`}
      ...
      {isOpen && (
        <div
          className="absolute top-full left-0 mt-1.5 min-w-[180px] bg-background/95 backdrop-blur-xl border border-border/50 rounded-xl shadow-2xl shadow-black/10 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150"
          onMouseEnter={() => setIsOpen(true)}
