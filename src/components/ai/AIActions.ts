...
const notesActions: AIAction[] = [
  {
    id: 'notes-summarize',
    label: 'Summarize',
    description: '3-sentence summary of the note',
    icon: 'Summary',
    buildPrompt: (ctx) => {
      const content = ctx.content || '';
      const title = ctx.title || 'Untitled';
      return `Summarize this note titled "${title}" in 3 concise sentences. Capture the main topic, key points, and conclusion:\n\n${content.substring(0, 4000)}`;
    },
  },
  {
    ...
    buildPrompt: (ctx) => {
      const content = ctx.content || '';
      return `Improve the grammar, spelling, and clarity of this text. Preserve the original meaning and style. Output the corrected version only:\n\n${content.substring(0, 4000)}`;
    },
  },
  {
    ...
    buildPrompt: (ctx) => {
      const content = ctx.content || '';
      const title = ctx.title || 'Untitled';
      return `Reformat this note titled "${title}" as a well-structured technical documentation page. Use markdown headings, code blocks, tables, and bullet points where appropriate:\n\n${content.substring(0, 4000)}`;
    },
  },
];
